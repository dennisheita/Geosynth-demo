...
const GeologicalCard = ({ data, onClick, isSelected = false, displayType }: GeologicalCardProps) => {
  ...
  const getStatusColor = (status: string) => {
    ...
    switch (status) {
      case 'ACTIVE': return 'bg-[hsl(0,0%,95%)]/20 text-[hsl(0,0%,95%)]';
      case 'SURVEYED': return 'bg-[hsl(0,0%,67%)]/20 text-[hsl(0,0%,67%)]';
      case 'PENDING': return 'bg-[hsl(0,0%,67%)]/20 text-[hsl(0,0%,67%)]';
      case 'RESTRICTED': return 'bg-[hsl(0,0%,3%)]/20 text-[hsl(0,0%,67%)]';
    ...
    >
      <div className="relative">
        ...
        <div className="relative z-10 p-5">
          ...
          <h2 className="text-xl font-bold text-[hsl(0,0%,95%)] mb-2">{displayInfo.title}</h2>
          ...
          <div className="space-y-3 mb-5">
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="text-[hsl(0,0%,95%)] text-xs font-medium">SURVEY</span>
                <span className="text-[hsl(0,0%,67%)] text-xs">{data.status}</span>
              </div>
              <div className="w-full h-1.5 bg-[hsl(0,0%,3%)]/80 rounded-full overflow-hidden">
                <div className="h-full bg-[hsl(0,0%,95%)]/70 rounded-full transition-all duration-300" style={{width: getProgressWidth(data.status)}}></div>
              </div>
            </div>
          </div>
